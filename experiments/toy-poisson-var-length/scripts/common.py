...
class Preset:
    ...
    def generate(self):
        ...
        for label, rate in enumerate(self.rates):
            scale = 1 / rate
            timestamps.append(np.cumsum(np.random.exponential(scale, size=self.max_length)))
            labels.append(np.full([self.max_length], label))
        # 2. Merge sequences and truncate to the specified length.
        timestamps = np.concatenate(timestamps)
        labels = np.concatenate(labels)
        order = np.argsort(timestamps)
        length = np.random.randint(self.min_length, self.max_length + 1)
        timestamps = timestamps[order][:length].tolist()
        labels = labels[order][:length].tolist()
