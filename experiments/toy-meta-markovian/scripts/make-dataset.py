...
def parse_args():
    ...
    parser.add_argument("--n-presets", help="The number of presets", type=int, default=3)
    ...
    parser.add_argument("--chunk-size", help="A single chunk size", type=int, default=16)
    parser.add_argument("--n-chunks", help="Sequence length in chunks", type=int, default=16)
    parser.add_argument("--temperature", help="Transition probabilities temperature", type=float, default=0.05)
...
def main(args):
    ...
    df = spark.createDataFrame(
        [(i, timestamps, labels, target)
         for i, (timestamps, labels, target) in enumerate([model.generate()[:3] for _ in range(args.size)])],
        StructType([
