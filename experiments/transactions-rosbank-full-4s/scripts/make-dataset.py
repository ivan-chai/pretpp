...
def train_val_test_split(transactions, targets):
    ...
    trainset_unsup = transactions.filter(transactions["id"].isin(train_unsup_ids))
    trainset_sup = transactions.filter(transactions["id"].isin(train_sup_ids))
    return trainset_unsup.persist(), trainset_sup.persist(), valset.persist(), testset.persist()
...
def main(args):
    ...
    # Split.
    train_unsup, train_sup, val, test = train_val_test_split(transactions, targets)
    # Revert compatibility cast.
    train_unsup = train_unsup.withColumn("id", F.col("id").cast("int"))
    train_sup = train_sup.withColumn("id", F.col("id").cast("int"))
    val = val.withColumn("id", F.col("id").cast("int"))
    test = test.withColumn("id", F.col("id").cast("int"))

    train_unsup_path = os.path.join(args.root, "train_unsup.parquet")
    train_sup_path = os.path.join(args.root, "train_sup.parquet")
    val_path = os.path.join(args.root, "val.parquet")
    test_path = os.path.join(args.root, "test.parquet")
    print(f"Dump train_unsup with {train_unsup.count()} records to {train_unsup_path}")
    dump_parquet(train_unsup, train_unsup_path, n_partitions=32)
    print(f"Dump train_sup with {train_sup.count()} records to {train_sup_path}")
    dump_parquet(train_sup, train_sup_path, n_partitions=32)
