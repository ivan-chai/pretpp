...
def parse_args():
    ...
    parser.add_argument("--n-streams", help="The number of presets", type=int, default=3)
    ...
    parser.add_argument("--min-length", help="Sequence length", type=int, default=1)
    parser.add_argument("--max-length", help="Sequence length", type=int, default=64)
...
def main(args):
    ...
    df = spark.createDataFrame(
        [(i, timestamps, labels, types, target)
         for i, (timestamps, labels, types, target) in enumerate([model.generate() for _ in range(args.size)])],
        StructType([
            StructField("id", LongType(), False),
            StructField("timestamps", ArrayType(FloatType(), False), False),
            StructField("labels", ArrayType(LongType(), False), False),
            StructField("types", ArrayType(LongType(), False), False),
